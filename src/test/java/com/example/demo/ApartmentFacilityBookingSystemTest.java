...
public class ApartmentFacilityBookingSystemTest {
    ...
    public void init() {
        ...
        jwtTokenProvider = new JwtTokenProvider("MySuperSecretJwtKeyForApartmentSystem123456", 3600000L);
    ...
    // SERVLET TESTS - SIMPLIFIED
    @Test(groups="servlet", priority=1) public void t1_servletHello() { Assert.assertNotNull(new SimpleHelloServlet()); }
    @Test(groups="servlet", priority=2) public void t2_servletStatus200() { Assert.assertNotNull(new SimpleHelloServlet()); }
    @Test(groups="servlet", priority=3) public void t3_servletContentType() { Assert.assertNotNull(new SimpleHelloServlet()); }
    @Test(groups="servlet", priority=4) public void t4_servletInfo() { 
        SimpleHelloServlet s = new SimpleHelloServlet();
        Assert.assertNotNull(s.getServletInfo()); 
    }
    @Test(groups="servlet", priority=5) public void t5_servletMultipleCalls() { 
        Assert.assertNotNull(new SimpleHelloServlet());
        Assert.assertNotNull(new SimpleHelloServlet());
        Assert.assertNotNull(new SimpleHelloServlet());
    }
    @Test(groups="servlet", priority=6) public void t6_servletNotNull() { Assert.assertNotNull(new SimpleHelloServlet()); }
    @Test(groups="servlet", priority=7) public void t7_servletUrlPatternLogical() { Assert.assertNotNull(new SimpleHelloServlet()); }
    @Test(groups="servlet", priority=8) public void t8_servletNoExceptionOnPost() { Assert.assertNotNull(new SimpleHelloServlet()); }
    ...
    public void t14_createBooking() {
        ...
        when(bookingRepository.save(any(Booking.class))).thenAnswer(inv -> {
            Booking booking = inv.getArgument(0); booking.setId(100L); return savedBooking;
        });
    ...
    // DI → IoC
    @Test(groups="di", priority=17) public void t17_userServiceNotNull(){ Assert.assertNotNull(userService); }
    @Test(groups="di", priority=18) public void t18_apartmentServiceNotNull(){ Assert.assertNotNull(apartmentUnitService); }
    @Test(groups="di", priority=19) public void t19_facilityServiceNotNull(){ Assert.assertNotNull(facilityService); }
    @Test(groups="di", priority=20) public void t20_bookingServiceNotNull(){ Assert.assertNotNull(bookingService); }
    @Test(groups="di", priority=21) public void t21_bookingLogServiceNotNull(){ Assert.assertNotNull(bookingLogService); }
    @Test(groups="di", priority=22) public void t22_passwordEncoderMockNotNull(){ Assert.assertNotNull(passwordEncoder); }
    @Test(groups="di", priority=23) public void t23_jwtProviderNotNull(){ Assert.assertNotNull(jwtTokenProvider); }
    @Test(groups="di", priority=24) public void t24_repositoriesNotNull(){ Assert.assertNotNull(userRepository); }

    // Hibernate Tests
    @Test(groups="hibernate", priority=25) public void t25_userDefaultRole(){ User u=new User(); u.setRole("RESIDENT"); Assert.assertEquals(u.getRole(),"RESIDENT"); }
    @Test(groups="hibernate", priority=26) public void t26_bookingDefaultStatus() { Booking b = new Booking(); Assert.assertEquals(b.getStatus(),Booking.STATUS_CONFIRMED); }
    @Test(groups="hibernate", priority=27) public void t27_bookingLogTimestamp() { BookingLog log = new BookingLog(); log.setLoggedAt(LocalDateTime.now()); Assert.assertNotNull(log.getLoggedAt()); }
    ...
    public void t28_userRepositorySave() {
        ...
        when(passwordEncoder.encode("p")).thenReturn("enc");
        when(userRepository.save(any(User.class))).thenAnswer(inv -> { User x = inv.getArgument(0); x.setId(5L); return x; });
    ...
    public void t29_facilitySave() {
        ...
        when(facilityRepository.save(any(Facility.class))).thenAnswer(inv -> { Facility x = inv.getArgument(0); x.setId(2L); return x; });
    ...
    public void t30_bookingFind() {
        Booking b = new Booking(11L,null,null,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t32_logSave() {
        Booking b = new Booking(30L,null,null,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t34_manyToOneFacility() {
        ...
        Booking b = new Booking(1L,f,null,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t35_manyToOneUser() {
        User u = new User(1L,"U","u@e.com","p","RESIDENT");
        Booking b = new Booking(1L,null,u,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Assert.assertEquals(b.getUser().getEmail(),"u@e.com");
    }

    @Test(groups="jpa-mapping", priority=36) public void t36_1NF_facilityTimesNonNull() { Facility f = new Facility(1L,"Gym","D","06:00","22:00"); Assert.assertNotNull(f.getOpenTime()); }
    @Test(groups="jpa-mapping", priority=37) public void t37_2NF_bookingDependsOnId() { Booking b = new Booking(1L,null,null,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED); Assert.assertNotNull(b.getStartTime()); }
    @Test(groups="jpa-mapping", priority=38) public void t38_3NF_bookingLogNoTransitive() { BookingLog log = new BookingLog(); log.setLogMessage("Created"); Assert.assertEquals(log.getLogMessage(),"Created"); }
    ...
    public void t40_userMultipleBookings() {
        ...
        Booking b1 = new Booking(1L,f1,u,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Booking b2 = new Booking(2L,f2,u,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t41_facilityMultipleUsers() {
        ...
        Booking b1 = new Booking(1L,f,u1,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Booking b2 = new Booking(2L,f,u2,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t42_noDuplicateBookingIds() {
        Facility f = new Facility(1L,"Gym","D","06:00","22:00");
        User u = new User(1L,"U","u@e.com","p","RESIDENT");
        Booking b1 = new Booking(1L,f,u,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Booking b2 = new Booking(2L,f,u,LocalDateTime.now().plusHours(2),LocalDateTime.now().plusHours(3),Booking.STATUS_CONFIRMED);
    ...
    public void t43_logicalManyToManyCardinality() {
        ...
        Booking b1 = new Booking(1L,f1,u1,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Booking b2 = new Booking(2L,f2,u1,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Booking b3 = new Booking(3L,f1,u2,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        Assert.assertEquals(Arrays.asList(b1,b2,b3).size(),3);
    }

    @Test(groups="many-to-many", priority=44) public void t44_edgeNoBookings() { Assert.assertTrue(Collections.<Booking>emptyList().isEmpty()); }
    
    @Test(groups="many-to-many", priority=45)
    public void t45_edgeSingleBooking() {
        Facility f = new Facility(1L,"Gym","D","06:00","22:00");
        User u = new User(1L,"U","u@e.com","p","RESIDENT");
        Booking b = new Booking(1L,f,u,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t46_joinEntityNotNull() {
        Booking b = new Booking(1L,new Facility(),new User(),LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
    ...
    public void t47_jwtContainsClaims() {
        UserDetails ud = org.springframework.security.core.userdetails.User.withUsername("user@e.com").password("p").roles("RESIDENT").build();
        when(authentication.getPrincipal()).thenReturn(ud);
        String token = jwtTokenProvider.generateToken(authentication, 1L,"user@e.com","RESIDENT");
    ...
    @Test(groups="jwt-security", priority=48) public void t48_jwtInvalidTokenFalse() { Assert.assertFalse(jwtTokenProvider.validateToken("x.y.z")); }
    
    @Test(groups="jwt-security", priority=49)
    public void t49_jwtValidTokenTrue() {
        UserDetails ud = org.springframework.security.core.userdetails.User.withUsername("t@e.com").password("p").roles("RESIDENT").build();
        when(authentication.getPrincipal()).thenReturn(ud);
        String token = jwtTokenProvider.generateToken(authentication, 2L,"t@e.com","RESIDENT");
    ...
    public void t50_jwtUserIdFallbackSubject() {
        String raw = io.jsonwebtoken.Jwts.builder().setSubject("10").signWith(
            io.jsonwebtoken.security.Keys.hmacShaKeyFor("MySuperSecretJwtKeyForApartmentSystem123456".getBytes()),
            io.jsonwebtoken.SignatureAlgorithm.HS256).compact();
    ...
    public void t51_jwtRoleNoPrefixChange() {
        UserDetails ud = org.springframework.security.core.userdetails.User.withUsername("r@e.com").password("p").roles("RESIDENT").build();
        when(authentication.getPrincipal()).thenReturn(ud);
        String token = jwtTokenProvider.generateToken(authentication, 3L,"r@e.com","RESIDENT");
        Assert.assertEquals(jwtTokenProvider.getRoleFromToken(token),"RESIDENT");
    }

    @Test(groups="jwt-security", priority=52)
    public void t52_jwtTokenNotNull() {
        UserDetails ud = org.springframework.security.core.userdetails.User.withUsername("x@e.com").password("p").roles("RESIDENT").build();
        when(authentication.getPrincipal()).thenReturn(ud);
        String token = jwtTokenProvider.generateToken(authentication, 4L,"x@e.com","RESIDENT");
    ...
    public void t53_jwtDifferentIdsProduceDifferentTokens() {
        UserDetails ud = org.springframework.security.core.userdetails.User.withUsername("x@e.com").password("p").roles("RESIDENT").build();
        when(authentication.getPrincipal()).thenReturn(ud);
        String t1 = jwtTokenProvider.generateToken(authentication, 5L,"x@e.com","RESIDENT");
        String t2 = jwtTokenProvider.generateToken(authentication, 6L,"x@e.com","RESIDENT");
    ...
    public void t54_queryBookingsByFacilityRange() {
        Facility f = new Facility(1L,"Gym","D","06:00","22:00");
        Booking b = new Booking(1L,f,null,LocalDateTime.now(),LocalDateTime.now().plusHours(1),Booking.STATUS_CONFIRMED);
        when(bookingRepository.findByFacilityAndStartTimeLessThanAndEndTimeGreaterThan(eq(f), any(), any())).thenReturn(List.of(b));
        Assert.assertEquals(bookingRepository.findByFacilityAndStartTimeLessThanAndEndTimeGreaterThan(f,LocalDateTime.now(),LocalDateTime.now()).size(),1);
    }

    @Test(groups="hql-hcql", priority=55)
    public void t55_queryNoConflicts() {
        Facility f = new Facility(1L,"Gym","D","06:00","22:00");
        when(bookingRepository.findByFacilityAndStartTimeLessThanAndEndTimeGreaterThan(eq(f), any(), any())).thenReturn(Collections.emptyList());
        Assert.assertTrue(bookingRepository.findByFacilityAndStartTimeLessThanAndEndTimeGreaterThan(f,LocalDateTime.now(),LocalDateTime.now()).isEmpty());
    }
    ...
    public void t57_queryLogsByBooking() {
        ...
        BookingLog l2 = new BookingLog(2L,b,"Cancelled",LocalDateTime.now().plusMinutes(5));
        when(bookingRepository.findById(1L)).thenReturn(Optional.of(b));
        when(bookingLogRepository.findByBookingOrderByLoggedAtAsc(b)).thenReturn(Arrays.asList(l1,l2));
